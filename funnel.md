...
- [LIVE] **nanodirectory.info** — entry 361 in directory.json (verified Sep 18)
- [LIVE] **agents-launch.lovable.app** — auto-indexed (verified Sep 18)
- [LIVE] **agentmrr.ai** — 160+ mentions found on rendering (verified Sep 18)
- [LIVE] **libhunt.com** — auto-indexed (2 URLs)
- All 11 pending directories still pending (earliest Sep 20-22)

## x402 Ecosystem listings
- **x402-foundation docs** — TWO prepared PR branches pushed to fork:
...
- req2 (GH PR scope, filed Sep 15): still open — customer action needed. GraphQL: Resource not accessible by personal access token
- Weekly X post slot: opens Sep 22 (last technical update Sep 15)

## Next actions
- Sep 20: re-check 5+ day directories (agents.net, theagentrank.com)
- Sep 22: weekly X post when slot opens
- When req2 arrives: x402-foundation/x402 docs PR first (strongest target with current docs)
- When req1 arrives: publish to PyPI (\`uv build\` + publish + verify)
- Per-run: verify fork branches still alive, check onchain-agents #11
